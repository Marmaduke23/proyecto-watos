document.addEventListener("DOMContentLoaded", () => {
    ...
    async function fetchRecommendationsWiki(nombre) {
        const spinner = document.getElementById("wiki-spinner");
        spinner.style.display = "block"; // mostrar spinner
        try {
            const res = await fetch(`/recomendar_wiki?nombre=${encodeURIComponent(nombre)}`);
            const data = await res.json();
            renderWikiRecommendations(data.recomendaciones);
        } catch (err) {
            console.error("Error al obtener recomendaciones:", err);
        } finally {
    ...
    function renderWikiRecommendations(recommendations) {
        const wikiRecommendationsDiv = document.getElementById("wiki-recommendations");
        console.log(recommendations);

        wikiRecommendationsDiv.innerHTML = recommendations.map(r => `
            <div class="col-md-4 mb-3">
                <div class="card card-hover h-100 p-3">
                    <h5 class="card-title text-success fw-bold">
                        <a href="${r.wikibase}" target="_blank">${r.name}</a>
                    </h5>
                    <p class="card-text">${""}</p>
                    ...
                    <div class="d-flex flex-wrap gap-2 mb-2">
                        <span class="badge bg-warning text-dark">Cal: ${r.calories}</span>
                        <span class="badge bg-info text-dark">Prot: ${r.protein}g</span>
                        <span class="badge bg-danger text-white">Grasa: ${r.totalFat}g</span>
                        <span class="badge bg-secondary text-white">Carbs: ${r.carbs}g</span>
                    </div>
                    ...
                    <div>
                        ${r.nutritionalSeals && r.nutritionalSeals.length > 0 
                            ? r.nutritionalSeals.map(s => `<span class="badge bg-danger text-white">${s}</span>`).join(" ")
                            : `<span class="text-muted">Sin sellos</span>`
                        }
                    </div>
                </div>
